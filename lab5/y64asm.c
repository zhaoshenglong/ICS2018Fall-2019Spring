...
symbol_t *find_symbol(char *name)
{
	if (symtab == NULL)
		return NULL;
	symbol_t *symtmp = symtab->next;
    while (symtmp) {
		if (strncmp(symtmp->name, name, strlen(name)) == 0 ) {
			return symtmp;	
		}
		symtmp = symtmp->next;
	} 
...
void add_reloc(char *name, bin_t *bin)
{
    ...
    reloc_t *reloctmp = (reloc_t *)malloc(sizeof(reloc_t));
	reloctmp->y64bin = bin;
	reloctmp->name = name;

    /* add the new reloc_t to relocation table */
	reloctmp->next = reltab->next;
	reltab->next = reloctmp;
}
...
parse_t parse_instr(char **ptr, instr_t **inst)
{
	...
	instr_t *instmp;
	if( (instmp = find_instr (*ptr) )!= NULL && *ptr != NULL){
		(*ptr) += instmp->len;
		*inst = instmp;
		return PARSE_INSTR;
	}	
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
	if( IS_REG(*ptr) ){
		const reg_t *regtmp = find_register( *ptr );
		if ( regtmp == NULL )
			return PARSE_ERR;
		else{
			(*ptr) += regtmp->namelen;
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	char tmp = (**ptr);
	(**ptr) = '\0';
	char *nametmp = (char*)malloc(len+1);
	strcpy(nametmp, ((*ptr)-len));
	(**ptr) = tmp;
	nametmp[len] = '\0';
	(*name) = nametmp;
...
*           call SUM  #invoke SUM function */
	...
	if(inst->bytes == 1){
		...
		byte_t reg_b = HPACK(regid, REG_NONE);
		if(strncmp(inst->name, "pushq", 5) == 0 || strncmp(inst->name, "popq", 4) == 0){
			line->y64bin.codes[1] = reg_b;
			line->y64bin.bytes = inst->bytes;
			return line->type;
		}
		else if(parse_delim(&ptr, ',') == PARSE_ERR){
			line->type = TYPE_ERR;
			err_print("Invalid ','");
			return line->type;
		}
		else if(parse_reg(&ptr, &regid) == PARSE_ERR){
			line->type = TYPE_ERR;
			err_print("Invalid REG");
			return line->type;
		}
		reg_b = HPACK(HIGH(reg_b), regid);
		line->y64bin.codes[1] = reg_b;
		line->y64bin.bytes = inst->bytes;
		return line->type;
	}
		...
		long value;
		parse_t parse_sign = parse_data(&ptr, &name, &value);
		if (parse_sign == PARSE_ERR){
			line->type = TYPE_ERR;
			return line->type;
		}
		else if (parse_sign == PARSE_SYMBOL){
		...
		line->y64bin.codes[0] = inst->code;
		if(parse_sign == PARSE_DIGIT){
			err_print("Invalid Symbol");
			line->type = PARSE_ERR;
			return line->type;
		}
		if(parse_sign == PARSE_SYMBOL){
			add_reloc(name, &(line->y64bin));
		}
		line->y64bin.bytes = inst->bytes;
		return line->type;
	}
	else if (inst->bytes == 10){
		if(strncmp(inst->name, "irmovq", 6) == 0) {
			long value;
			parse_t parse_sign = parse_imm(&ptr, &name, &value);
			if(parse_sign == PARSE_ERR){
			...
			line->y64bin.codes[1] = HPACK(REG_NONE, regid);
			line->y64bin.bytes = inst->bytes;
			return line->type;
		}
		if(strncmp(inst->name, "rmmovq", 6) == 0) { 
			regid_t regid;
			if (parse_reg(&ptr, &regid) == PARSE_ERR){
				line->type = TYPE_ERR;
				err_print("Invalid REG");
			...
			long value = 0;
			if(parse_mem(&ptr, &value, &regid) == PARSE_ERR){
				line->type = TYPE_ERR;
		...
		if(strncmp(inst->name, "mrmovq", 6) == 0) {
			long value = 0;
			regid_t regid;
			if(parse_mem(&ptr, &value, &regid) == PARSE_ERR){
				line->type = TYPE_ERR;
			...
			byte_t reg_b = HPACK(REG_NONE, regid);
			if( parse_delim(&ptr, ',') == PARSE_ERR ){
				line->type = TYPE_ERR;
				err_print("Invalid ','");
				return line->type;
		...
		else if(strncmp(inst->name, ".align", 6) == 0){
			long value;
			if(parse_digit(&ptr, &value) == PARSE_ERR){
				line->type = TYPE_ERR;
				return line->type;
...
int relocate(void)
{
    ...
    while (rtmp) {
		...
		if(rtmp->y64bin->bytes == 9){
			int64_t val = symtmp->addr;
			rtmp->y64bin->codes[1] = (val&0xff);
			rtmp->y64bin->codes[2] = ((val>>8)&0xff);
			rtmp->y64bin->codes[3] = ((val>>16)&0xff);
			rtmp->y64bin->codes[4] = ((val>>24)&0xff);
			rtmp->y64bin->codes[5] = ((val>>32)&0xff);
			rtmp->y64bin->codes[6] = ((val>>40)&0xff);
			rtmp->y64bin->codes[7] = ((val>>48)&0xff);
			rtmp->y64bin->codes[8] = ((val>>56)&0xff);
		}
		else if(rtmp->y64bin->bytes == 10){
			int64_t val = symtmp->addr;
			rtmp->y64bin->codes[2] = (val&0xff);
			rtmp->y64bin->codes[3] = ((val>>8)&0xff);
			rtmp->y64bin->codes[4] = ((val>>16)&0xff);
			rtmp->y64bin->codes[5] = ((val>>24)&0xff);
			rtmp->y64bin->codes[6] = ((val>>32)&0xff);
			rtmp->y64bin->codes[7] = ((val>>40)&0xff);
			rtmp->y64bin->codes[8] = ((val>>48)&0xff);
			rtmp->y64bin->codes[9] = ((val>>56)&0xff);
		}
		else if(rtmp->y64bin->bytes == 8){
			int64_t val = symtmp->addr;
			rtmp->y64bin->codes[0] = (val&0xff);
			rtmp->y64bin->codes[1] = ((val>>8)&0xff);
			rtmp->y64bin->codes[2] = ((val>>16)&0xff);
			rtmp->y64bin->codes[3] = ((val>>24)&0xff);
			rtmp->y64bin->codes[4] = ((val>>32)&0xff);
			rtmp->y64bin->codes[5] = ((val>>40)&0xff);
			rtmp->y64bin->codes[6] = ((val>>48)&0xff);
			rtmp->y64bin->codes[7] = ((val>>56)&0xff);
		}
		else if(rtmp->y64bin->bytes == 4){
			int64_t val = symtmp->addr;
			rtmp->y64bin->codes[0] = (val&0xff);
			rtmp->y64bin->codes[1] = ((val>>8)&0xff);
			rtmp->y64bin->codes[2] = ((val>>16)&0xff);
...
int binfile(FILE *out)
{
	...
	line_t *linetmp = line_head->next;
	int64_t preaddr = 0, prebytes = 0;
	while(linetmp){
		if(linetmp->type == TYPE_INS){
			if(linetmp->y64bin.bytes == 0 && linetmp->y64bin.addr != vmaddr){
				char padding_byte[1];
				padding_byte[0] = 0;
				int padding_count = linetmp->y64bin.addr - preaddr - prebytes;
				if(padding_count > 0)
					for(int i = 0; i < padding_count; i++)
						if(fwrite(padding_byte, 1, 1, out) < 1)
							return -1;		
			}
			size_t write_bytes = fwrite(linetmp->y64bin.codes, 1, linetmp->y64bin.bytes, out);
			if (write_bytes < linetmp->y64bin.bytes)
				return -1;
			prebytes = linetmp->y64bin.bytes;
			preaddr = linetmp->y64bin.addr;
		}
