...
long long gen_d_srcA()
{
    return (((if_id_curr->icode) == (I_RRMOVQ) || (if_id_curr->icode) == 
        ...
        (I_POPQ) || (if_id_curr->icode) == (I_RET) || (if_id_curr->icode)
         == (I_POP2)) ? (REG_RSP) : (REG_NONE));
}
...
long long gen_d_dstM()
{
    return (((if_id_curr->icode) == (I_MRMOVQ) || (if_id_curr->icode) == 
        (I_POPQ) || (if_id_curr->icode) == (I_POP2)) ? (if_id_curr->ra) : 
      (REG_NONE));
}
...
long long gen_aluA()
{
    return (((id_ex_curr->icode) == (I_RRMOVQ) || (id_ex_curr->icode) == 
        ...
        (I_RET) || (id_ex_curr->icode) == (I_POPQ) || (id_ex_curr->icode)
         == (I_POP2)) ? 8 : 0);
}
...
{
    return (((mem_wb_curr->icode) == (I_POP2)) ? (mem_wb_curr->destm) : (
        (mem_wb_curr->icode) == (I_POPQ)) ? (mem_wb_curr->deste) : (
        (mem_wb_curr->destm) != (REG_NONE)) ? (mem_wb_curr->destm) : 
      (mem_wb_curr->deste));
}

long long gen_w_valE()
{
    return (((mem_wb_curr->icode) == (I_POP2)) ? (mem_wb_curr->valm) : (
        (mem_wb_curr->icode) == (I_POPQ)) ? (mem_wb_curr->vale) : (
        (mem_wb_curr->destm) != (REG_NONE)) ? (mem_wb_curr->valm) : 
      (mem_wb_curr->vale));
}
