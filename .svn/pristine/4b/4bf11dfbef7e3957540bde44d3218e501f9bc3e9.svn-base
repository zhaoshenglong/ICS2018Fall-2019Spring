...
# id : 515030910241
# description:
# 1. add IIADDQ instruction
# 2. modify the hcl of D_stall and add load forwarding 
# 3. modify the D_bubble and E_bubble and f_predPC to always not taken
#    when find the misprediction insert a bubble 
####################################################################
...
word f_pc = [
	...
	M_icode == IJXX && !M_Cnd && W_icode != IRMMOVQ   : M_valA;
...
word f_predPC = [
	f_icode == IJXX && M_icode == IOPQ && D_icode == IRMMOVQ : f_valP;
...
bool F_stall =
	...
	&& !(E_dstM == d_srcA && D_icode in {IPUSHQ, IRMMOVQ} && E_dstM != d_srcB )
	||
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && M_icode != IRMMOVQ) ||
	# Stalling at fetch while ret passes through pipeline
