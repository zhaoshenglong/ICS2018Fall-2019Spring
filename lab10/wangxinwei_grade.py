#!/usr/bin/env python2
'''
    This file is forfed from Wang XinWei. Thanks for his help. 
'''
import pexpect
import random
import signal
...
def restart_client(client):
    if client != None:
        client.close()
        client.wait()
        return start_client(client.target_port)
    else:
        return start_client(outside_proxy)

def restart_sc(server, client):
    server_port, server = restart_server(server)
    client = restart_client(client)
    return server, server_port, client

def restart_env(server, client, proxy):
    proxy = restart_proxy(proxy)
    server, server_port, client = restart_sc(server, client)
...
def do_part1_4(server, server_port, client, proxy, in_part1_7=False):
    ...
    if not in_part1_7:
        server, server_port, client, proxy = restart_env(server, client, proxy)
    else:
        server, server_port, client = restart_sc(server, client)
    req = HttpRequest("POST", server_ip, server_port, path, body)
...
def do_part2_1(server, server_port, client, proxy):
    ...
    for i in xrange(PART2_1_ROUND):
        ...
        for j in xrange(len(cut_point) / 2):
            ...
            server2, server_port2, client2 = restart_sc(server2, client2)
...
class ConcurrentPayload(object):
    def __init__(self, server, proxy, req, resp):
        self.proxy = proxy
        if proxy == None:
            self.client = start_client(outside_proxy)
        else:
